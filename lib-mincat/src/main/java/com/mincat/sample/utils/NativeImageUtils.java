...
/**
 * @author Gin
 * @描述 本地图片加载器, 采用的是异步解析本地图片，单例模式利用getInstance()获取NativeImageLoader实例
 * 调用loadNativeImage()方法加载本地图片，此类可作为一个加载本地图片的工具类
 */
public class NativeImageUtils {

    private LruCache<String, Bitmap> mMemoryCache;
    private static NativeImageUtils mInstance = new NativeImageUtils();
    ...
    public static NativeImageUtils getInstance() {
        return mInstance;
    }
    ...
    // 加载视频缩略图，对图片不进行裁剪
    public Bitmap loadNativeVideoThumbnails(final String path,
                                            final NativeImageCallBack mCallBack) {

        return this.loadNativeVideoThumbnails(path, null, mCallBack);
    }

    // 此方法来加载本地视频缩略图，这里的mPoint是用来封装ImageView的宽和高，我们会根据ImageView控件的大小来裁剪Bitmap
    // 如果你不想裁剪图片，调用loadNativeVideoThumbnails(final String path, final
    // NativeImageCallBack mCallBack)来加载
    public Bitmap loadNativeVideoThumbnails(final String path,
