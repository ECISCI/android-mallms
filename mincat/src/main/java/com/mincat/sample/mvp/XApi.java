...
import com.mincat.sample.custom.progress.ProgressHelper;
...
import com.mincat.sample.mvp.utils.Kits;
import com.mincat.sample.mvp.utils.MCLogInterceptor;
...
public class XApi {
    ...
    private OkHttpClient getClient(String baseUrl, NetProvider provider) {
        ...
        if (handler != null) {
            builder.addInterceptor(new MinCatInterceptor(handler));
        }
        ...
        if (provider.configLogEnable()) {
            MCLogInterceptor logInterceptor = new MCLogInterceptor();
            ...
            public Publisher<T> apply(Flowable<T> upstream) {
                return upstream.flatMap(new Function<T, Publisher<T>>() {
                    ...
                    public Publisher<T> apply(T model) throws Exception {
                        ...
                        if (model.responseDataIsNull()) {

                            return Flowable.error(new NetError(model.getErrCode(), model.getErrorMsg(), NetError.NoDataError));

                        } else if (model.authError()) {

                            return Flowable.error(new NetError(model.getErrCode(), model.getErrorMsg(), NetError.AuthError));
