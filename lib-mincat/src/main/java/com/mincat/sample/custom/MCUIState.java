 ...
 * @author Gin
 */
public class MCUIState extends FrameLayout {

    View loadingView, errorView, errorNetView, emptyView, contentView;
    ...
    int loadingLayoutId, errorLayoutId, errorNetLayoutId, emptyLayoutId, contentLayoutId;
    ...
    public MCUIState(Context context) {
        this(context, null);
    }

    public MCUIState(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MCUIState(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        setupAttrs(context, attrs);
    }

    private void setupAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MCUIState);
        loadingLayoutId = typedArray.getResourceId(R.styleable.MCUIState_x_loadingLayoutId, RES_NONE);
        errorLayoutId = typedArray.getResourceId(R.styleable.MCUIState_x_errorLayoutId, RES_NONE);
        errorNetLayoutId = typedArray.getResourceId(R.styleable.MCUIState_x_errorNetLayoutId, RES_NONE);
        emptyLayoutId = typedArray.getResourceId(R.styleable.MCUIState_x_emptyLayoutId, RES_NONE);
        contentLayoutId = typedArray.getResourceId(R.styleable.MCUIState_x_contentLayoutId, RES_NONE);
        typedArray.recycle();
    }
    ...
    protected void onFinishInflate() {
        ...
        if (childCount > 1) {
            throw new IllegalStateException("UIStateController can only host 1 elements");
        } else {
            ...
            if (errorNetLayoutId != RES_NONE) {
                errorNetView = inflate(getContext(), errorNetLayoutId, null);
        ...
        if (newState != oldState) {
            ...
            switch (newState) {
                ...
                case STATE_NET_ERROR:
                    notifyStateChange(oldState, newState, errorNetView);
                    break;
                case STATE_EMPTY:
    ...
    private View getDisplayView(int oldState) {
        ...
        if (oldState == STATE_NET_ERROR) return errorNetView;
    ...
    public MCUIState loadingView(View loadingView) {
    ...
    public MCUIState errorView(View errorView) {
    ...
    public MCUIState errorNetView(View errorView) {
        if (this.errorNetView != null) {
            removeView(this.errorNetView);
        }
        this.errorNetView = errorView;
        addView(this.errorNetView);
        this.errorNetView.setVisibility(GONE);
        return this;
    }

    public MCUIState emptyView(View emptyView) {
    ...
    public MCUIState contentView(View contentView) {
